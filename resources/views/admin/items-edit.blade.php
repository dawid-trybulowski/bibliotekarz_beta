...
<div class="col s9" style="color:#fff">
    <h3>Edycja egzemplarza {{$data['items']->id}}</h3>
</div>
</div>
</div>
<div id="app">
    ...
    <div class="row">
        <form class="col s12" method="POST" action="{{route('admin-items-edit')}}">
            {{ csrf_field() }}
            <input type="hidden" name="id" value="{{$data['items']->id}}">
            <div class="col s12">
                <div class="form-group{{ $errors->has('bookId') ? ' has-error' : '' }}">
                    ...
                    <div class="input-field col s4">
                        <input placeholder='ID pozycji' id="bookId" type="text" name="bookId" class="validate"
                               value="{{ old('bookId') ? old('bookId') : $data['items']->book_id}}"
                ...
                <div class="col s7 input-field centered_new" style="margin-top:2%">
                    <h6 id="bookData">{{$data['items']->title}} {{$data['items']->author}}</h6>
                </div>
                ...
                <div class="form-group{{ $errors->has('isbn') ? ' has-error' : '' }}">
                    ...
                    <div class="input-field col s6">
                        <input id="isbn" type="text" name="isbn" class="validate"
                               value="{{ old('isbn') ? old('isbn') : $data['items']->isbn}}">
                ...
                <div class="form-group{{ $errors->has('publicationYear') ? ' has-error' : '' }}">
                    ...
                    <div class="input-field col s6">
                        <input id="publicationYear" type="text" name="publicationYear" class="validate"
                               value="{{ old('publicationYear') ? old('publicationYear') : $data['items']->publication_year}}">
